...
完整字段说明、五家提供商模板、测试、故障转移和对账流程见 [上游模型接入与渠道管理指南](./04-upstream-channel-guide.md)。
...
推荐初始优先级：已审核第三方主用 `100`，官方兜底 `50`，权重均为 `1`。当前版本按较高优先级优先路由，但保存前仍需以后台字段提示为准。优先级只控制路由，不会自动计算成本或利润。
